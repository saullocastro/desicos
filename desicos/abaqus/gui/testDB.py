...
def cc_form2dict(db, form):
    ...
    others = ['r', 'h', 'alphadeg', 'elem_type', 'betadeg', 'omegadeg',
              'numel_r', 'axial_displ']
    for k in others:
...
class TestDB(AFXDataDialog):
    ...
    def __init__(self, form):
        ...
        AFXTextField(geomVA, 8, 'R:', form.rKw, opts=AFXTEXTFIELD_FLOAT)
        ...
        FXLabel(matFrame, '')
        FXLabel(matFrame, 'For isotropic, make E22, G12, G13 and G23 as blank fields')
        FXLabel(matFrame, '')
        ...
        #
        # Tabs / Model / Analysis Steps
        #
        FXTabItem(modelBook, 'Analysis Steps', None, TAB_LEFT)
        nlHF    = FXHorizontalFrame(modelBook,
                    opts=FRAME_RAISED|FRAME_SUNKEN)
        # initial step
        nlVF = FXVerticalFrame(nlHF)
        pngpath = os.path.join(DAHOME, 'gui', 'icons', 'bcs3.png')
        icon = afxCreatePNGIcon(pngpath)
        FXLabel(nlVF, '')
        FXLabel(nlVF, 'Initial Step', icon)
        FXLabel(nlVF, '')
        FXLabel(nlVF, '')
        FXLabel(nlVF, 'OBS: Simply supported\n'+\
                     'modelling still needs\n'+\
                     'to be checked...')
        FXLabel(nlVF, '')
        FXLabel(nlVF, '')
        FXLabel(nlVF, 'Boundary Conditions:')
        nlHF2   = FXHorizontalFrame(nlVF, opts=LAYOUT_CENTER_X)
        bctopGB = FXGroupBox(nlHF2, 'TOP', opts=FRAME_GROOVE)
        FXCheckButton(bctopGB, 'U1' , form.topU1Kw)
        FXCheckButton(bctopGB, 'U2' , form.topU2Kw)
        FXCheckButton(bctopGB, 'U3' , form.topU3Kw)
        FXCheckButton(bctopGB, 'UR1', form.topUR1Kw)
        FXCheckButton(bctopGB, 'UR2', form.topUR2Kw)
        FXCheckButton(bctopGB, 'UR3', form.topUR3Kw)
        bcbotGB = FXGroupBox(nlHF2, 'BOT', opts=FRAME_GROOVE)
        FXCheckButton(bcbotGB, 'U1' , form.botU1Kw)
        FXCheckButton(bcbotGB, 'U2' , form.botU2Kw)
        FXCheckButton(bcbotGB, 'U3' , form.botU3Kw)
        FXCheckButton(bcbotGB, 'UR1', form.botUR1Kw)
        FXCheckButton(bcbotGB, 'UR2', form.botUR2Kw)
        FXCheckButton(bcbotGB, 'UR3', form.botUR3Kw)
        # perturbation load step
        FXVerticalSeparator(nlHF)
        nlVF = FXVerticalFrame(nlHF)
        pngpath = os.path.join(DAHOME, 'gui', 'icons', 'pl4.png')
        icon = afxCreatePNGIcon(pngpath)
        FXLabel(nlVF, '')
        FXLabel(nlVF, 'Perturbation Load Step', icon)
        FXLabel(nlVF, '')
        FXCheckButton(nlVF, 'Artificial Damping', form.artificial_damping1Kw)
        FXLabel(nlVF, '')
        AFXTextField(nlVF, 8, 'Damping Factor:', form.damping_factor1Kw,
                     opts=AFXTEXTFIELD_FLOAT)
        nlVA = AFXVerticalAligner(nlVF)
        ...
        icon = afxCreatePNGIcon(pngpath)
        FXLabel(nlVF, '')
        FXLabel(nlVF, 'Axial Compression Step', icon)
        FXLabel(nlVF, '')
        FXCheckButton(nlVF, 'Include axial compression:', form.axial_includeKw)
        FXLabel(nlVF, '')
        FXCheckButton(nlVF, 'Separate load steps (one for perturbation,\n' +\
                            'another for axial compression):',
                            form.separate_load_stepsKw)
        FXHorizontalSeparator(nlVF)
        FXLabel(nlVF, '')
        AFXTextField(nlVF, 8, 'Axial compression:', form.axial_displKw,
                     AFXTEXTFIELD_FLOAT)
        FXLabel(nlVF, '')
        FXHorizontalSeparator(nlVF)
        FXCheckButton(nlVF, 'Artificial Damping', form.artificial_damping2Kw)
        FXLabel(nlVF, '')
        AFXTextField(nlVF, 8, 'Damping Factor:', form.damping_factor2Kw, opts=AFXTEXTFIELD_FLOAT)
        nlVA = AFXVerticalAligner(nlVF)
        ...
        AFXTextField(outputFrame, 8, 'Number of points per Step',
                     form.time_pointsKw, opts=AFXTEXTFIELD_INTEGER)
        ...
        for k in ['pl', 'd', 'ax', 'lbmi', 'cut']:
            ...
            impVF = FXVerticalFrame(impBook,
                                    LAYOUT_FILL_Y|FRAME_RAISED|FRAME_SUNKEN)
            impHF = FXHorizontalFrame(impVF,
                                    FRAME_RAISED|FRAME_SUNKEN|LAYOUT_FILL_X)
            self.imp_spinners[k] = AFXSpinner(impHF, 2, labelSpinners[k],
                                                imp_numKw[k])
            #if k == 'pl':
            #    self.imp_spinners[k].setRange(1, maxIMP)
            #else:
            self.imp_spinners[k].setRange(0, maxIMP)
            FXHorizontalSeparator(impVF)
            impHF = FXHorizontalFrame(impVF, FRAME_RAISED|FRAME_SUNKEN)
        ...
        FXCheckButton(impVF, 'Apply already when the study is created', form.imp_ms_stdKw).disable()
        FXLabel(impVF, '')
        FXLabel(impVF, '')
        ...
        FXCheckButton(impVF, 'Apply already when the study is created', form.imp_t_stdKw).disable()
        FXLabel(impVF, '')
        FXLabel(impVF, '')
        ...
        FXCheckButton(execVF2,\
            'Use multiple cpus (some licenses do not allow this feature)',
            form.mult_cpusKw)
        FXLabel(execVF2, '')
        FXCheckButton(execVF2,\
    ...
    def processUpdates(self):
        ...
        if self.exec_std.getState() == STATE_DOWN:
            ...
            self.logcount = 10000
            mult_cpus = form.mult_cpusKw.getValue()
            command = 'import __main__\n'      +\
                      '__main__.stds[ "%s" ].write_inputs()\n' % std_name
            sendCommand(command)
            reload(gui_commands)
            gui_commands.run_study(std_name, mult_cpus, form.use_job_stopperKw.getValue())
        # save study
